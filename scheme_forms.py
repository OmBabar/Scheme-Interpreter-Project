...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        "*** YOUR CODE HERE ***"
        var = do_lambda_form(Pair(signature.rest, expressions.rest), env)       
        env.define(signature.first, var)
...
def do_and_form(expressions, env):
    ...
    while (expressions): 
        ...
        # if statement to check if expressions is last
        if (expressions.rest == nil):
            return scheme_eval(expressions.first, env, True)
        first_val = scheme_eval(expressions.first, env)
        # else: 
        #     first_val = scheme_eval(expressions.first, env)
        if (not is_scheme_true(first_val)): 
            return first_val
...
def make_let_frame(bindings, env):
    ...
    while (bindings is not nil): 
        current_list = bindings.first
        
        validate_form(current_list, 2, 2)

        name = current_list.first
        val = scheme_eval(current_list.rest.first, env)
